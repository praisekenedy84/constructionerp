...
export default function DepartmentsIndex() {
    const { departments, filters, auth } = usePage<DepartmentsIndexProps>().props;
    ...
    function submit(e: FormEvent) {
        ...
        put(`/requisitions/departments/${editingId}`, {
            onSuccess: () => {
                reset();
                setOpen(false);
                setEditingId(null);
    ...
    function archiveDepartment(department: DepartmentRow) {
        if (
            !confirm(
                `Remove department “${department.name}”? If it is already used on requisitions it will be deactivated instead.`,
            )
        ) {
            return;
        }
        router.delete(`/requisitions/departments/${department.id}`);
    }
    ...
    return (
        <AppShell title="Departments">
            ...
            <div className="space-y-6">
                <PageHeader
                    title="Departments"
                    description="Define the department labels staff pick when creating requisitions."
                    actions={
                        canCreate ? (
                ...
                <ListToolbar
                    baseUrl="/requisitions/departments"
                    filters={filters}
                ...
                <DataPanel title="Defined Departments" noPadding>
                            ...
                            {rows.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="px-6 py-12 text-center text-slate-500">
                                        No departments yet. Create the options staff should see when
            ...
            <Dialog
                ...
                description="The department name appears in the requisition form dropdown."
            >
                <form onSubmit={submit} className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            placeholder="e.g. Site Operations, Procurement, Plant"
                            required
                        />
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="department_sort_order">Sort order</Label>
                        <Input
